...
public class PathFinderManager : MonoBehaviourSingleton<PathFinderManager>
{
    ...
    public void SetPathfindingStrategy(PathfindingStrategy strategy)
    {
        if (isCalculating)
        {
            Debug.LogWarning("Cannot change algorithm while calculating a path.");
            return;
        }

        pathfindingStrategy = strategy;
        Debug.Log("Pathfinding strategy changed to: " + strategy);
